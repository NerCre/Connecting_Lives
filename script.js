...
(() => {
  ...
  function renderStatusGrid() {
    ...
    let situations = master.situations.slice();
    if (state.mode === 'emergency') {
      situations = situations.filter((s) => s.defaultAction === 'emergency');
      // In emergency mode, still keep “その他” (just in case)
      const other = master.situations.find((s) => s.id === 'other');
      if (other && !situations.some((x) => x.id === other.id)) situations.push(other);
    }
    ...
    for (const s of situations) {
      ...
      btn.innerHTML = `
        <div class="icon" aria-hidden="true">${escapeHtml(s.icon || '')}</div>
        <div class="label">
  ...
  function renderCompanyList() {
    ...
    for (const c of master.companies) {
      ...
      btn.addEventListener('click', () => {
        ...
        renderKanaBar();
        renderPersonList('あ'); // default group
  ...
  function renderPersonList(groupLabel) {
    ...
    for (const p of people) {
      ...
      btn.addEventListener('click', () => {
        ...
        if (s && s.requiresBody) {
          // go to body
          $('#bodyTitle').textContent = s.label + ' - 部位選択';
          nav.show('view-body');
        } else {
          // go to result
          buildResultPreview();
