...
(() => {
   ...
   *  - Tap on map to choose nearest zone
   *  - Or search/select from the list
   *  ========================= */
  ...
  function initMapOverlay() {
    ...
    const apply = () => {
      const w = img.naturalWidth || MAP_BASE_W;
      const h = img.naturalHeight || MAP_BASE_H;
      mapScale = { sx: w / MAP_BASE_W, sy: h / MAP_BASE_H };
  ...
  function scaledPoints(poly) {
    return poly.map(([x, y]) => [x * mapScale.sx, y * mapScale.sy]);
  }
  ...
  function openMapModal() {
    ...
    // Set image src (in case files are served from GitHub)
    const img = $('#yardMap');
    if (img) img.setAttribute('src', MAP_IMAGE_SRC);
    ...
    // Ensure overlay is drawn for the new image
    initMapOverlay();
  ...
  function setMapSelection(area) {
    ...
    // Update marker (centroid)
    if (marker && img && area) {
      const w = img.naturalWidth || MAP_BASE_W;
      const h = img.naturalHeight || MAP_BASE_H;
      const x = (area.cx * mapScale.sx) / w;
      const y = (area.cy * mapScale.sy) / h;
      marker.style.left = (x * 100).toFixed(2) + '%';
      marker.style.top = (y * 100).toFixed(2) + '%';
      marker.classList.remove('hidden');
    }

    // Refresh overlay + list highlight
    renderMapOverlay();
  ...
  function handleMapTap(e) {
    ...
    const xBase = (nx * w) / (mapScale.sx || 1);
    const yBase = (ny * h) / (mapScale.sy || 1);

    const hit = findAreaAtPoint(xBase, yBase);
    const picked = hit || findNearestArea(xBase, yBase);
...
function renderCompanyList() {
  ...
  function wireGlobalEvents() {
    ...
    $('#yardMap')?.addEventListener('click', (e) => handleMapTap(e));
    $('#yardMap')?.addEventListener('touchstart', (e) => handleMapTap(e), { passive: true });
