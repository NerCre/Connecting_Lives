...
(() => {
  ...
  // Normalized anchors (0-1) based on map_grid.png (2481x1754)
  const MAP_ZONES = [
    { name: '北定盤2', x: 0.2096, y: 0.3421 },
    { name: 'ピース切断場', x: 0.1330, y: 0.3278 },
    { name: '道具置場', x: 0.1814, y: 0.3193 },
    { name: '施設作業場', x: 0.1491, y: 0.2965 },
    { name: '旧ガスセンター工場', x: 0.3104, y: 0.3335 },
    { name: 'B棟', x: 0.2499, y: 0.4105 },
    { name: '足場材置場（北）', x: 0.3789, y: 0.4048 },
    { name: '北定盤1', x: 0.1250, y: 0.4675 },
    { name: '120t JC', x: 0.2096, y: 0.4561 },
    { name: 'A棟', x: 0.2579, y: 0.4960 },
    { name: '170t JC', x: 0.2257, y: 0.5358 },
    { name: 'DOCK', x: 0.0806, y: 0.5131 },
    { name: 'SUB定盤', x: 0.4031, y: 0.3706 },
    { name: 'SUB工場', x: 0.4635, y: 0.3706 },
    { name: '事務所', x: 0.4635, y: 0.4390 },
    { name: '守衛室', x: 0.3749, y: 0.5017 },
    { name: '食堂・協力業者ハウス', x: 0.5320, y: 0.3193 },
    { name: 'ブロック置場', x: 0.5885, y: 0.4333 },
    { name: '鋼材・SUB材置場', x: 0.6247, y: 0.1539 },
    { name: '曲げ定盤', x: 0.5965, y: 0.3193 },
    { name: 'パイプ置場', x: 0.6651, y: 0.4276 },
    { name: '機電装課/艤装品置場', x: 0.5526, y: 0.4276 },
    { name: '艤装岸壁', x: 0.1290, y: 0.6556 },
    { name: '南定盤1', x: 0.2217, y: 0.6271 },
    { name: '70t JC', x: 0.1693, y: 0.7355 },
    { name: 'C棟', x: 0.2418, y: 0.6967 },
    { name: '艤装品置場', x: 0.1854, y: 0.8666 },
    { name: 'スクラップ場', x: 0.2378, y: 0.8894 },
    { name: '300t JC', x: 0.3023, y: 0.5530 },
    { name: '南定盤2', x: 0.3386, y: 0.6100 },
    { name: '南定盤3', x: 0.4232, y: 0.6043 },
    { name: '足場材置場（南）', x: 0.4837, y: 0.5872 },
    { name: '艤装定盤', x: 0.4232, y: 0.6613 },
    { name: '加工場', x: 0.3708, y: 0.7070 },
    { name: 'パイプ工場', x: 0.4313, y: 0.7412 },
    { name: '電気室・コンプレッサー室', x: 0.4837, y: 0.6556 },
  ];

  let mapSelected = null; // {name,x,y}
  ...
  function openMapModal() {
    ...
    const img = $('#yardMap');
    if (img && !img.getAttribute('src')) img.setAttribute('src', MAP_IMAGE_SRC);
  ...
  function setMapSelection(zone) {
    mapSelected = zone;
    // Update label
    const sel = $('#mapSelectedLabel');
    if (sel) sel.textContent = zone?.name || '未選択';

    // Update marker
    const marker = $('#mapMarker');
    if (marker && zone) {
      marker.style.left = (zone.x * 100).toFixed(2) + '%';
      marker.style.top = (zone.y * 100).toFixed(2) + '%';
  ...
  function findNearestZone(normX, normY) {
    let best = null;
    let bestD = Infinity;
  ...
  function handleMapTap(e) {
    ...
    const nearest = findNearestZone(nx, ny);
    if (nearest) setMapSelection(nearest);
  }
