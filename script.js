...
(() => {
  ...
  function initOnePage() {
    ...
    function updateCallButton() {
      const ok = stateOne.conscious !== null && stateOne.breathing !== null && stateOne.location !== null;
      callBtn.disabled = !ok;
      // ボタンの色クラスを整理
      callBtn.classList.remove('btn-primary', 'btn-secondary', 'btn-emergency');
      if (ok) {
    ...
    function wireSeg(id, prop) {
      ...
      wrap.querySelectorAll('button').forEach((btn) => {
        btn.addEventListener('click', () => {
          ...
          stateOne[prop] = btn.getAttribute('data-val');
          updateCallButton();
        });
      });
    }
    ...
    // 事故区分: pictogram 定義
    const accidentDefs = [
      { key: 'bleeding_major', label: '大量出血', icon: '🩸' },
    ...
    if (btnSms) {
      btnSms.addEventListener('click', () => {
        ...
        // メッセージ生成
        const parts = [];
        parts.push(`意識: ${stateOne.conscious || '不明'}`);
        parts.push(`呼吸: ${stateOne.breathing || '不明'}`);
        const loc = stateOne.location;
        parts.push(`場所: ${loc ? (loc.name || (loc.unknown ? '不明' : '')) : '未選択'}`);
        if (stateOne.accidents.length) parts.push(`事故区分: ${stateOne.accidents.join(',')}`);
        if (stateOne.accidentNote) parts.push(`補足: ${stateOne.accidentNote}`);
        if (stateOne.victim) parts.push(`被災者: ${stateOne.victim.name || (stateOne.victim.unknown ? '不明' : '')}`);
...
function renderCompanyList() {
  ...
  function wireGlobalEvents() {
    ...
    if (startEm) startEm.addEventListener('click', (ev) => {
      ...
      // トップバーの戻る／再起動ボタンは無効にする（戻るでホームに戻るように設定）
