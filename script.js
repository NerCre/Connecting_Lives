...
(() => {
  ...
  function normalizeEmails(str) {
    // Master上の「送信先」はSMS用の電話番号として扱う（ハイフンありで統一）
    ...
    const toHalf = (t) => t.replace(/[０-９]/g, (d) => String.fromCharCode(d.charCodeAt(0) - 0xFEE0));
    const toHyphen = (t) => t.replace(/[－ー―−]/g, '-');

    const fmt = (t) => {
      let v = toHyphen(toHalf(t)).replace(/[^0-9-]/g, '');
  ...
  // Comma-separated token list (used for includeEmergency/includeObserve settings)
  ...
  // Format phone list string (comma-separated) using normalizeEmails() rule
  function formatPhoneListString(str) {
    return normalizeEmails(str).join(', ');
  }

  function attachPhoneAutoFormat(inputEl) {
    if (!inputEl) return;
    // Do formatting on blur to avoid cursor jump while typing
    inputEl.addEventListener('blur', () => {
      const formatted = formatPhoneListString(inputEl.value);
      if (formatted !== (inputEl.value || '').trim()) inputEl.value = formatted;
    });
  }
  ...
  function defaultMaster() {
    return {
      ...
      globalContacts: {
        safetyHQ: '090-0000-0001',
        rescueTeam: '090-0000-0002',
        ambulanceCenter: '090-0000-0003',
      },
      // 送信先範囲（マスタでON/OFF）
      sendScope: {
        safetyHQ: true,
      ...
      companies: [
        { id: 'own', name: '自社', emails: ['090-0000-0101', '090-0000-0102'] },
        { id: 'a', name: 'A造船', emails: ['090-0000-0201', '090-0000-0202'] },
        { id: 'b', name: 'B株式会社', emails: ['090-0000-0301'] },
      ],
      locations: [
      ...
      accidentTypes: ACCIDENT_TYPES_DEFAULT,
      situations: [
        {
          id: 'unconscious',
          label: '意識なし',
  ...
  function loadMaster() {
    ...
    try {
      ...
      merged.accidentTypes = mergeById(def.accidentTypes || [], parsed.accidentTypes);
  ...
  function defaultWizardState() {
    return {
      ...
      reportTime: nowHHMM(),
      occurKind: 'discover', // 'occur' | 'discover'
   ...
   *  ========================= */
  const ACCIDENT_TYPES_DEFAULT = [
    { id: 'high_place', label: '高所', icon: '🏗️' },
    { id: 'confined_space', label: '閉所', icon: '🚪' },
    { id: 'water', label: '溺水', icon: '🌊' },
    { id: 'fire', label: '火災', icon: '🔥' },
    { id: 'electric', label: '感電', icon: '⚡' },
    { id: 'heavy_object', label: '重量物', icon: '🧱' },
  ];

  function goWizardStep(stepKey, { push = true } = {}) {
    const id = WIZ[stepKey] || WIZ.report;
    nav.show(id, { push });
    saveSession({ ...state, nav: nav.stack });
  }
  ...
  function onViewShown(viewId) {
    // One-screen emergency report
    if (viewId === WIZ.report) {
      renderReport();
      return;
    }

    // Other legacy views (kept for backward compatibility)
  ...
  function renderReportMeta() {
    const t = $('#reportTime');
    if (t) t.value = state.wiz.reportTime || '';
    ...
    if (seg) {
      ...
      buttons.forEach((b) => {
        ...
        const active = state.wiz.occurKind === val;
  ...
  function isReportReadyToCall() {
    const triage = state.wiz.triage;
    const loc = state.wiz.location;
    const hasLoc = !!(loc && (loc.unknown || (loc.name && String(loc.name).trim())));
    return !!(triage.conscious && triage.breathing && hasLoc);
  }

  function updateActionButtons() {
    const callBtn = $('#btnCallAmbulance');
    if (callBtn) {
      const ready = isReportReadyToCall();
      callBtn.disabled = !ready;
      callBtn.classList.toggle('armed', ready);
    }
  }

  function renderSmsPreview() {
    const pre = $('#smsPreview');
    if (!pre) return;
    const draft = buildWizardSmsDraft();
    pre.textContent = draft.body || '';
  }
  ...
  function renderWizardAccident() {
    ...
    if (wrap) {
      const defs = (master.accidentTypes && master.accidentTypes.length ? master.accidentTypes : ACCIDENT_TYPES_DEFAULT);
      wrap.innerHTML = '';
      defs.forEach((d) => {
        const btn = document.createElement('button');
        btn.type = 'button';
        btn.className = 'pict-btn';
        btn.dataset.acc = d.id;
        btn.setAttribute('aria-pressed', 'false');
        btn.innerHTML = `<div class="pict-icon" aria-hidden="true">${escapeHtml(d.icon || '')}</div><div class="pict-label">${escapeHtml(d.label || '')}</div>`;
        wrap.appendChild(btn);
      });
    }

    const types = new Set(state.wiz.accident.types || []);
    $$('#accidentChips .pict-btn').forEach((c) => {
  ...
  // --- SMS draft for MVP (SMS only) ---
  function buildWizardSmsDraft() {
    const triage = state.wiz.triage || {};
    const loc = state.wiz.location || {};
    const acc = state.wiz.accident || {};
    const v = state.wiz.victim || {};
    ...
    const locLabel = loc.unknown ? '不明' : (loc.name || '未設定');
    const victimLabel = staff?.name || v.name || (v.unknown ? '不明' : '未設定');

    const time = (state.wiz.reportTime || '').trim() || ((state.wiz.startedAt || '').split(' ')[1] || '');
    const kind = state.wiz.occurKind === 'occur' ? '発生' : '発見';

    const accLabels = (acc.types || []).map(getAccidentLabel).filter(Boolean);
    const accLine = accLabels.length ? accLabels.join(' / ') : '未選択';

    const lines = [];
    lines.push('【緊急通報】');
    lines.push(`時刻: ${time || 'ー'}（${kind}）`);
    lines.push(`場所: ${locLabel}`);
    lines.push(`意識: ${yesNoUnknownLabel(triage.conscious)}`);
    lines.push(`呼吸: ${yesNoUnknownLabel(triage.breathing)}`);
    lines.push(`事故: ${accLine}`);
    if ((acc.note || '').trim()) lines.push(`補足: ${acc.note.trim()}`);
    lines.push(`被災者: ${victimLabel}`);
    if (company?.name) lines.push(`所属: ${company.name}`);
    if (staff?.id) lines.push(`職員ID: ${staff.id}`);
    lines.push('—');
    lines.push('※命をツナグ（MVP）より');

    return { to, body: lines.join('\n') };
  }

  function openSmsDraft(draft) {
    const nums = (draft.to || []).filter(Boolean);
    const body = draft.body || '';
    const encBody = encodeURIComponent(body);
    const numPart = nums.join(',');
    // iOS/Androidの差異を吸収するため、?&body を使う
    const url = `sms:${numPart}?&body=${encBody}`;
  ...
  const MAP_IMAGES = {
    all: 'assets/map_overview.png',
    a1: 'assets/map_area1.png',
    a2: 'assets/map_area2.png',
    a3: 'assets/map_area3.png',
  };
  ...
  const MAP_AREA_POLYS = {
    a1: [[0,0],[2300,0],[2300,900],[1500,1150],[0,1150]],
    a2: [[0,1150],[1500,1150],[2300,900],[2300,2339],[0,2339]],
    a3: [[2300,0],[3307,0],[3307,2339],[2300,2339],[1500,1150],[2300,900]],
  };
  ...
  const MAP_AREAS_RAW = [
    ...
    {
      name: '曲げ定盤',
      poly: [[2320, 650], [2560, 650], [2560, 1230], [2320, 1230]]
    },
    {
      name: 'ブロック置場',
      poly: [[2080, 710], [2900, 710], [2920, 1210], [2580, 1210], [2580, 1360], [2140, 1360]]
    },
    {
      name: 'パイプ置場',
      poly: [[2580, 1210], [2920, 1210], [2920, 1360], [2580, 1360]]
    },
    {
      name: '食堂・協力業者ハウス',
      poly: [[2090, 600], [2450, 600], [2450, 710], [2090, 710]]
    },
    {
      name: 'SUB定盤',
      poly: [[1250, 620], [1700, 560], [2050, 650], [2140, 820], [1800, 920], [1380, 860], [1250, 740]]
    },
    {
      name: 'SUB工場',
      poly: [[1760, 780], [2040, 780], [2040, 910], [1760, 910]]
    },
    {
      name: '事務所',
      poly: [[1760, 920], [2040, 920], [2040, 1000], [1760, 1000]]
    },
    {
      name: '南定盤3',
      poly: [[1700, 1300], [1950, 1300], [1950, 1450], [1700, 1450]]
    },
    {
      name: '南定盤2',
      poly: [[1280, 1300], [1700, 1300], [1700, 1450], [1280, 1450]]
    },
    {
      name: '加工場',
      poly: [[1470, 1460], [1630, 1460], [1630, 1600], [1470, 1600]]
    },
    {
      name: 'パイプ工場',
      poly: [[1640, 1460], [1860, 1460], [1860, 1600], [1640, 1600]]
    },
    {
      name: '電気室・コンプレッサー室',
      poly: [[1860, 1440], [2070, 1500], [2200, 1640], [2000, 1730], [1780, 1620]]
    },
    {
      name: '北定盤2',
      poly: [[460, 600], [650, 600], [650, 750], [500, 820], [420, 760]]
    },
    {
      name: 'ピース切断場',
      poly: [[260, 560], [410, 560], [410, 650], [260, 650]]
    },
    {
      name: '道具置場',
      poly: [[430, 570], [520, 570], [520, 650], [430, 650]]
    },
    {
      name: '施設作業場',
      poly: [[260, 520], [420, 520], [420, 560], [260, 560]]
    },
    {
      name: '旧ガスセンター工場',
      poly: [[980, 590], [1230, 590], [1230, 680], [980, 680]]
    },
    {
      name: 'B棟',
      poly: [[450, 720], [1100, 720], [1100, 860], [450, 860]]
    },
    {
      name: '北定盤1',
      poly: [[280, 850], [500, 850], [500, 980], [280, 980]]
    },
    {
      name: 'A棟',
      poly: [[560, 900], [1460, 900], [1460, 1020], [560, 1020]]
    },
    {
      name: 'DOCK',
      poly: [[210, 990], [320, 990], [320, 1090], [210, 1090]]
    },
    {
      name: '建造船',
      poly: [[420, 1040], [1440, 1040], [1440, 1260], [420, 1260]]
    },
    {
      name: '艤装岸壁',
      poly: [[630, 1260], [730, 1260], [730, 1950], [630, 1950]]
    },
    {
      name: '70t JC',
      poly: [[740, 1260], [820, 1260], [820, 1950], [740, 1950]]
    },
    {
      name: 'C棟',
      poly: [[850, 1300], [1030, 1300], [1030, 1920], [850, 1920]]
    },
    {
      name: '艤装品置場',
      poly: [[820, 1780], [1030, 1780], [1030, 1920], [820, 1920]]
    },
    {
      name: 'スクラップ場',
      poly: [[1040, 1850], [1180, 1850], [1180, 2050], [1040, 2050]]
    },
  ];
  ...
  function placeAreaKey(place){
    // Heuristic split that matches the current yard layout
    if (place.cx >= 1850) return 'a3';
    if (place.cy < 1050) return 'a1';
    return 'a2';
  }
  ...
  function computeAreaViewBox(areaKey){
    const items = MAP_PLACES.filter(p => p.areaKey === areaKey);
  ...
  function openVictimModal(){
    const modal = $('#victimModal');
    if (!modal) return;
    modal.classList.remove('hidden');
    const q = $('#victimSearch');
    if (q) q.value = '';
    renderWizardVictim();
    q?.focus?.();
  }
...
function renderCompanyList() {
  ...
  function renderAdminCompanies() {
    ...
    master.companies.forEach((c) => {
      ...
      div.innerHTML = `
        ...
        <div class="small">送信先（電話番号）: ${escapeHtml(emails)}</div>
        <div class="form-grid">
          <input data-k="name" value="${escapeHtml(c.name)}" />
          <input data-k="emails" type="tel" inputmode="tel" value="${escapeHtml(emails)}" />
          <button class="btn btn-secondary" data-act="save">保存</button>
          <button class="btn btn-secondary" data-act="del">削除</button>
        </div>
      `;

      // Auto hyphen formatting on blur
      attachPhoneAutoFormat(div.querySelector('input[data-k="emails"]'));

      div.querySelector('[data-act="save"]').addEventListener('click', () => {
        const name = div.querySelector('input[data-k="name"]').value.trim();
        const emInput = div.querySelector('input[data-k="emails"]');
        const formatted = formatPhoneListString(emInput.value);
        emInput.value = formatted;
        const em = normalizeEmails(formatted);
  ...
  function renderAdminSituations() {
    ...
    master.situations.forEach((s) => {
      ...
      div.querySelector('[data-act="save"]').addEventListener('click', () => {
        ...
        // includeEmergency/includeObserve are keys (tokens), not phone numbers
        s.includeEmergency = normalizeTokens(div.querySelector('input[data-k="includeEmergency"]').value);
        s.includeObserve = normalizeTokens(div.querySelector('input[data-k="includeObserve"]').value);
  ...
  function wireGlobalEvents() {
    ...
    // Admin: phone inputs auto-format (digits -> hyphen) on blur
    attachPhoneAutoFormat($('#newCompanyEmails'));
    attachPhoneAutoFormat($('#gcSafetyHQ'));
    attachPhoneAutoFormat($('#gcRescueTeam'));
    attachPhoneAutoFormat($('#gcAmbulance'));

    // Admin: add company
    $('#btnAddCompany').addEventListener('click', () => {
      const name = $('#newCompanyName').value.trim();
      const formatted = formatPhoneListString($('#newCompanyEmails').value);
      $('#newCompanyEmails').value = formatted;
      const emails = normalizeEmails(formatted);
      if (!name) return toast('会社名を入力してください');
    ...
    $('#btnSaveGlobalContacts').addEventListener('click', () => {
      // store as hyphenated phone list string
      master.globalContacts.safetyHQ = formatPhoneListString($('#gcSafetyHQ').value);
      master.globalContacts.rescueTeam = formatPhoneListString($('#gcRescueTeam').value);
      master.globalContacts.ambulanceCenter = formatPhoneListString($('#gcAmbulance').value);

      // reflect formatted values back to inputs
      $('#gcSafetyHQ').value = master.globalContacts.safetyHQ || '';
      $('#gcRescueTeam').value = master.globalContacts.rescueTeam || '';
      $('#gcAmbulance').value = master.globalContacts.ambulanceCenter || '';
    ...
    /** ===== One-screen report (view-report) events ===== */
    $('#reportTime')?.addEventListener('change', (e) => {
      if (!state.wiz) state.wiz = defaultWizardState();
      state.wiz.reportTime = e.target.value || state.wiz.reportTime;
      renderReportMeta();
      renderSmsPreview();
      updateActionButtons();
      saveSession({ ...state, nav: nav.stack });
    ...
    $('#btnCallAmbulance')?.addEventListener('click', () => {
      if (!isReportReadyToCall()) return;
      // Demo: 117 (time signal). Real operation: 119
    ...
    // Victim modal (name search)
    $('#btnOpenVictimModal')?.addEventListener('click', () => openVictimModal());
    $('#btnVictimClose')?.addEventListener('click', () => closeVictimModal());
    $('#btnVictimCancel')?.addEventListener('click', () => closeVictimModal());
    $('#btnVictimUnknown2')?.addEventListener('click', () => {
    ...
    $('#victimModal')?.addEventListener('click', (e) => {
      if (e.target?.id === 'victimModal') closeVictimModal();
    });

    // Accident pictogram toggles (works for both wizard & one-screen)
    $('#accidentChips')?.addEventListener('click', (e) => {
      const btn = e.target.closest('.pict-btn');
      if (!btn) return;
      ...
      state.wiz.accident.types = Array.from(types);
      renderWizardAccident();
      renderSmsPreview();
      updateActionButtons();
      saveSession({ ...state, nav: nav.stack });
    ...
    // Segmented selections (triage + occurKind)
    document.addEventListener('click', (e) => {
      ...
      if (!state.wiz) state.wiz = defaultWizardState();
      if (field === 'occurKind') {
        state.wiz.occurKind = val;
        renderReportMeta();
      } else {
        if (!state.wiz.triage) state.wiz.triage = { conscious: 'unknown', breathing: 'unknown', numbness: 'unknown' };
    ...
    $('#victimList')?.addEventListener('click', (e) => {
      ...
      updateActionButtons();
      saveSession({ ...state, nav: nav.stack });
      if (!$('#victimModal')?.classList.contains('hidden')) closeVictimModal();
    });
