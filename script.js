...
(() => {
  ...
  function renderWizardTriage() {
    ...
    function syncGroup(group) {
      const buttons = document.querySelectorAll(`.seg[data-group="${group}"] .seg-btn`);
      buttons.forEach((b) => {
  ...
  function renderWizardLocation() {
    ...
    $('#currentLocationLabel').textContent = loc.unknown ? '未設定' : (loc.name || '未設定');
    $('#currentLocationQr').textContent = loc.qr ? `QR: ${loc.qr}` : '';

    const list = $('#locationPickList');
    if (list) {
      list.innerHTML = '';
      ...
      if (items.length === 0) {
        ...
        for (const it of items) {
          ...
          b.addEventListener('click', () => {
            state.wiz.location = { qr: it.qr || '', name: it.name || '', unknown: false };
            saveSession({ ...state, nav: nav.stack });
  ...
  function renderWizardVictim() {
    ...
    wrap.textContent = companyName ? `${name}（${companyName}）` : name;
  ...
  function renderVictimSearchList(query) {
    ...
    for (const p of people) {
      ...
      b.addEventListener('click', () => {
        state.wiz.victim = { staffId: p.id, name: '', qr: state.wiz.victim.qr || '', unknown: false };
        saveSession({ ...state, nav: nav.stack });
        renderWizardVictim();
      });
      list.appendChild(b);
  ...
  function buildWizardPreview() {
    ...
    lines.push('');
    lines.push(`【事故区分】${(acc.types || []).length ? acc.types.join(' / ') : '未選択'}`);
  ...
  function renderWizardReview() {
    ...
    $('#sumTime').textContent = state.wiz.startedAt;
    $('#sumTriage').textContent = `意識: ${yesNoUnknownLabel(triage.conscious)} / 呼吸: ${yesNoUnknownLabel(triage.breathing)}`;
    $('#sumLocation').textContent = loc.unknown ? '不明' : (loc.name || '未設定');
    $('#sumAccident').textContent = (acc.types || []).length ? acc.types.join(' / ') : '未選択';
    $('#sumVictim').textContent = staff?.name || v.name || (v.unknown ? '不明' : '未設定');
    const to = p.to;
    $('#sumRecipients').textContent = to.length ? to.join(', ') : '未設定（管理画面で送信先を登録してください）';

    // show raw preview
    $('#mailTo').textContent = to.join(', ');
    $('#mailSubject').textContent = p.subject;
    $('#mailBody').textContent = p.body;

    // warnings
    const warns = [];
    if (loc.unknown) warns.push('場所が未確定です（場所QR/手動選択を推奨）');
    if (!triage.conscious || !triage.breathing) warns.push('意識/呼吸が未選択です');
    if (!(acc.types || []).length) warns.push('事故区分が未選択です');
    if (v.unknown) warns.push('被災者が不明です（ヘルメットQR/氏名検索を推奨）');
    $('#reviewWarnings').textContent = warns.length ? `注意: ${warns.join(' / ')}` : '';
  }
  ...
  async function startQrCamera() {
    // UI
    $('#qrVideoBox').classList.remove('hidden');
    $('#qrFallbackHint').classList.add('hidden');

    // Feature detection
    if (!('mediaDevices' in navigator) || !navigator.mediaDevices.getUserMedia) {
      $('#qrVideoBox').classList.add('hidden');
      $('#qrFallbackHint').classList.remove('hidden');
      $('#qrError').textContent = 'この端末/ブラウザではカメラ読み取りが利用できません。下の入力欄にQR文字列を貼り付けてください。';
      return;
    }
    ...
    if (!('BarcodeDetector' in window)) {
      $('#qrVideoBox').classList.add('hidden');
      $('#qrFallbackHint').classList.remove('hidden');
      $('#qrError').textContent = 'このブラウザではQR自動検出が利用できません。下の入力欄にQR文字列を貼り付けてください。';
      return;
    }
    ...
    try {
      ...
      $('#qrVideoBox').classList.add('hidden');
      $('#qrFallbackHint').classList.remove('hidden');
      $('#qrError').textContent = 'カメラの起動に失敗しました。権限設定を確認するか、下の入力欄にQR文字列を貼り付けてください。';
    }
  }
  ...
  function wireGlobalEvents() {
    ...
    $('#locationManualName')?.addEventListener('input', (e) => {
    ...
    $('#locationList')?.addEventListener('click', (e) => {
      ...
      $('#locationManualName').value = state.wiz.location.name || '';
      renderWizardLocation();
      saveSession({ ...state, nav: nav.stack });
    });
    ...
    $('#btnQrUseText')?.addEventListener('click', () => {
      const v = ($('#qrManualText')?.value || '').trim();
